...
class faculty_id(APIView):
    ...
    def delete(self,request,pk):
        faculty=Faculty.objects.get(pk=pk)
        faculty.delete()
        return Response({"faculty":"Faculty deleted successfully"},status=status.HTTP_204_NO_CONTENT)
...
class Books_id(APIView):
    ...
    def delete(self,request,pk):
        book=BookPublication.objects.get(pk=pk)
        book.delete()
        return Response({"content":" Book content deleted successfully"},status=status.HTTP_204_NO_CONTENT)
...
class confrence_id(APIView):
    def get(self,request,pk):
        try:
            ...
            return Response({'error':'Conefrence id not found'},status=status.HTTP_404_NOT_FOUND)
